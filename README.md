...
All of these are documented with OpenAPI; see [Exploring the API with Swagger
UI](#exploring-the-api-with-swagger-ui).
...
If you'd rather not use curl, the app serves interactive docs generated by
[springdoc-openapi](https://springdoc.org/) from the controllers:

- Swagger UI: <http://localhost:8081/swagger-ui.html>
- OpenAPI spec (JSON): <http://localhost:8081/v3/api-docs>

The endpoints are grouped as **Payment events**, **Audit log** and
**Notifications**. The `POST /api/payment-events` request body comes with
example values, so you can open it, click **Try it out** and **Execute**, then
run the `GET` endpoints to see where the event ended up. Change `eventType` to
`PaymentProcessed` or set `simulateFailure` to `true` to see the filtering and
dead-letter paths described above.

The spec is also useful outside the UI, for example to generate a client or
import the API into Postman.

Ports are offset from `order-entitlement-service` (`8081`, Postgres on `5435`,
LocalStack on `4567`, the debugger on `5006`) specifically so both projects can
run side by side without colliding.

### Attaching a debugger

The `app` container starts with the JVM debug agent enabled, listening on 5005
inside the container and published on **5006** on the host. It's `suspend=n`, so
the app boots normally whether or not anything is attached.

In IntelliJ: *Run > Edit Configurations > + > Remote JVM Debug*, host
`localhost`, port `5006`. In VS Code, an `attach` configuration of type `java`
with `"hostName": "localhost", "port": 5006`. Then set breakpoints as usual —
`PaymentEventPublisher.publish` and the two consumers' handlers are the
interesting places to start.

To run without the debug agent, set `JAVA_TOOL_OPTIONS=` (empty) in the
environment `docker compose` runs in, or drop the line from `docker-compose.yml`.

Debugging the app straight from the IDE instead (with only Postgres and
LocalStack in Docker) needs no debug port at all — run `EventFanoutApplication`
in debug mode with `AWS_ENDPOINT_OVERRIDE=http://localhost:4567` set.
...
- No auth/authz, and that includes the Swagger UI and `/v3/api-docs`, which are
  open. In a real deployment they'd be locked down or turned off with
